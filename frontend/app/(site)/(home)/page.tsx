...
export default async function HomePage() {
  ...
  /**
   * Each offer tile points at the category it talks about.
   *
   * `ctaUrl` is the CMS field and always wins — but its column is not
   * migrated yet, so until it is, the destination is read out of the tile's
   * own copy: "Révélez la beauté de vos cheveux" resolves to /shop/cheveux
   * against the real Categories tree. Before this, the href was hardcoded and
   * every tile led to the whole catalogue whatever it showed.
   *
   * The tree is the same fetch the navigation makes, deduplicated by Next's
   * request cache, so this costs no extra round trip. A tile whose copy names
   * no category keeps the catalogue link rather than being sent somewhere
   * plausible but wrong.
   */
  // Typed as CtaTile rather than inferred from the snapshot: data/home.ts is
  // regenerated by sync-cms and does not carry `ctaUrl` yet, so the inferred
  // shape has no such property to read.
  const categoryTree = await fetchCategoryTree().catch(() => EMPTY_CATEGORY_TREE);
  const withTileHrefs = (tiles: CtaTile[]): CtaTile[] =>
    tiles.map((tile) => ({
      ...tile,
      ctaUrl: tile.ctaUrl?.trim() || findCategoryHrefInText(categoryTree, tile.title, tile.eyebrow) || undefined,
    }));
  const offerTiles1 = withTileHrefs(ctaPair1);
  const offerTiles2 = withTileHrefs(ctaPair2);
  ...
  const sectionsByKey: Record<SectionKey, ReactNode> = {
    ...
    ctaPair1: <CtaPair tiles={offerTiles1} />,
    ...
    ctaPair2: <CtaPair tiles={offerTiles2} title="Nos sélections" />,
