      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          <CardContent>
            ...
            <div className="mt-1 text-2xl font-semibold text-foreground">{customer.orderCount}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="text-xs font-medium text-muted-foreground">Total dépensé</div>
            <div className="mt-1 text-2xl font-semibold text-foreground">{money(customer.totalSpent)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="text-xs font-medium text-muted-foreground">Téléphone</div>
            <div className="mt-1 text-lg font-medium text-foreground">{customer.phone || "—"}</div>
          </CardContent>
        </Card>
      </div>
            ...
            <tbody>
              {customer.orders.map((o) => (
                <tr key={o.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-5 py-3 text-muted-foreground">{new Date(o.createdAt).toLocaleDateString("fr-FR")}</td>
                  <td className="px-5 py-3">
                    <Badge>{ORDER_STATUS_LABELS[o.status]}</Badge>
                  </td>
                  <td className="px-5 py-3 text-right font-medium text-foreground">{money(o.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
